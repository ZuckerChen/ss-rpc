...
import com.ssrpc.core.network.RpcClient;
...
public class NettyClient implements RpcClient {
    ...
    private void cleanup() {
        try {
            ...
            for (CompletableFuture<RpcResponse> future : pendingRequests.values()) {
                future.completeExceptionally(new NetworkException("Client is shutting down"));
            }
    ...
    public CompletableFuture<RpcResponse> sendRequest(String address, RpcRequest request) {
        if (!started) {
            return CompletableFutureUtils.failedFuture(new NetworkException("Client not started"));
        }
        ...
        try {
            ...
            channel.writeAndFlush(request).addListener((ChannelFutureListener) channelFuture -> {
                if (!channelFuture.isSuccess()) {
                    ...
                    future.completeExceptionally(new NetworkException(
    ...
    @Override
    public RpcResponse sendRequestSync(String address, RpcRequest request) throws NetworkException {
        try {
            return sendRequest(address, request).get(config.getRequestTimeout(), TimeUnit.MILLISECONDS);
        } catch (TimeoutException e) {
            throw new NetworkException("Request timeout", e);
        } catch (ExecutionException e) {
            Throwable cause = e.getCause();
            if (cause instanceof NetworkException) {
                throw (NetworkException) cause;
            } else {
                throw new NetworkException("Request execution error", cause);
            }
        } catch (InterruptedException e) {
            Thread.currentThread().interrupt();
     ...
     * 获取或创建到指定地址的连接
     */
    private Channel getOrCreateChannel(String address) throws NetworkException {
        Channel channel = channelPool.get(address);
     ...
     * 创建到指定地址的连接
     */
    private Channel createChannel(String address) throws NetworkException {
        ...
        try {
            ...
            throw new NetworkException("Invalid port in address: " + address, e);
        }
        ...
        try {
            ...
            if (future.isSuccess()) {
                ...
                throw new NetworkException("Failed to connect to " + address, future.cause());
            }
            
        } catch (InterruptedException e) {
            Thread.currentThread().interrupt();
            throw new NetworkException("Connection interrupted: " + address, e);
        }
    }
